...
/************************
 * MM2K Bench – App.jsx
 * Version: v2025.12.25-21
 *
 * - Lokala profiler (konton) + delningskod (?k=...)
 * - Vercel Blob: spara / ladda / synka / historik (via /api/profiles/*)
 * - 14 pass, Failure Tests (#5,7,9,11,13) med låsning + FT‑stjärnor
 * - Popup‑quotes vid "Klart" och FT (guld/silver/brons)
 ************************/

/****************
 * Konstanter & utils
 ****************/
const APP_VERSION = "v2025.12.25-21";
...
function roundTo(x, step = 0.5) {
  const s = Number(step) || 0.5; return Math.round((Number(x) || 0) / s) * s;
}
 ...
 * Program & setbygge
 ****************/
// 14 pass. FT på 5,7,9,11,13. #14 är Max‑test.
const WORKOUTS = [
  { id: 1, name: "#1 – Bas A" },
  { id: 2, name: "#2 – Bas B" },
...
// Enkel procentschema. Avrundas till användarens kg‑steg.
function buildPrescription(workoutId, workingRmKg, step) {
  ...
  switch (workoutId) {
    ...
    case 4: // Tung singel
      ...
      add("Back‑off", 5, 0.70);
      break;
    case 5: case 7: case 9: case 11: case 13: { // Failure Test
      add("Block 1", 2, 0.87);
      add("Block 2", 5, 0.75);
      rows.push({ label: "Failure (AMRAP)", reps: "AMRAP", targetKg: f(0.75), kind: "failure" });
      break;
    }
    case 6: case 10: // Negativt set ~110%
      add("Block 1", 3, 0.85);
      add("Block 2", 3, 0.80);
      rows.push({ label: "Negativ 1x1", reps: 1, targetKg: roundTo(w * 1.10, step), kind: "negative" });
      break;
    case 8: case 12:
      add("Block 1", 2, 0.88);
      add("Block 2", 4, 0.78);
      ...
      break;
    case 14: // Max‑test
      add("Uppvärmning", 1, 0.90);
      rows.push({ label: "Max‑försök ~115%", reps: 1, targetKg: roundTo(w * 1.15, step), kind: "max" });
      break;
    default:
      add("Block 1", 3, 0.8);
      add("Block 2", 6, 0.75);
...
function calcFtDelta(reps) {
  ...
  return 0; // 4–7 ingen ändring
}
function starFromReps(val) {
  const n = Number(val);
...
function renderStar(kind, key) {
  switch (kind) {
    case "gold": return <span key={key} className="text-amber-500">★</span>;
    case "silver": return <span key={key} className="text-slate-400">★</span>;
    case "bronze": return <span key={key} className="text-orange-600">★</span>;
    default: return <span key={key} className="text-slate-300/60">☆</span>;
  }
}
...
export default function App() {
   ...
   * Quotes (popup)
   **************/
  function showQuote(kind, extraLine = "") {
    const q = getQuote(kind);
    const main = q?.text || "";
    const sub = q?.author ? `— ${q.author}` : extraLine || "";
    setToast({ type: "quote", msg: main, sub });
  }
  ...
  async function createAccount() {
    ...
    let blobKey = null; if (maybeCode.trim()) { blobKey = extractBlobKeyFromInput(maybeCode); if (!blobKey) alert("Koden/länken kunde inte tolkas."); }
    ...
    if (blobKey) {
      try { const r = await fetch(`/api/profiles/${blobKey}`); if (r.ok) { const data = await r.json(); if (Array.isArray(data.users)) setUsers(data.users); if (data.profileMeta) setMeta(data.profileMeta); setToast({ type: 'up', msg: 'Profil kopplad och laddad från server.' }); } }
      catch {}
    }
  }
  ...
  async function connectWithCodeFlow(prefilled) {
    ...
    if (!key) { alert('Koden/länken kunde inte tolkas.'); return; }
    const label = 'Delad profil';
    const acc = { id: uid(), label, pin: null, blobKey: key, createdAt: new Date().toISOString() };
    const next = [...accounts, acc]; setAccounts(next); setAccountId(acc.id);
    try { const r = await fetch(`/api/profiles/${key}`); if (r.ok) { const data = await r.json(); if (Array.isArray(data.users)) setUsers(data.users); if (data.profileMeta) setMeta(data.profileMeta); setToast({ type: 'up', msg: 'Profil kopplad och laddad från server.' }); } else { setToast({ type: 'note', msg: 'Koden är kopplad, men ingen serverprofil fanns ännu.' }); } } catch {}
  }

  // Fångar delningslänk ?k=...
  useEffect(() => {
    const k = new URLSearchParams(location.search).get("k");
    if (!k) return;
    const label = "Delad profil";
    const acc = { id: uid(), label, pin: null, blobKey: k, createdAt: new Date().toISOString() };
    setAccounts(prev => [...prev, acc]); setAccountId(acc.id);
  ...
  function markDone(workoutId, done = true, opts = {}) {
    ...
    if (done && !prev.done) {
      ...
      if (!opts.suppressQuote) showQuote("pass_done");
  ...
  function recomputeWorkingRm(excludeId = null) {
    if (!selected) return selected?.workingRmKg || 0;
    let w = Number(selected.oneRmKg) || 0;
    for (const id of FT_IDS) {
      if (id === excludeId) continue; const lg = selected.logs?.[id]; if (lg?.ftApplied && Number.isFinite(lg.ftDelta)) w += Number(lg.ftDelta);
    }
  ...
  function confirmFailure(workoutId) {
    ...
    markDone(workoutId, true, { forceRows: rowsNow, suppressQuote: true }); // auto‑Klart
    ...
    let kind = "ft_silver"; if (reps >= 8) kind = "ft_gold"; else if (reps <= 3) kind = "ft_bronze";
    const deltaStr = ftDelta > 0 ? `(+${ftDelta} kg)` : ftDelta < 0 ? `(${ftDelta} kg)` : "(±0 kg)";
    showQuote(kind, `FT: ${reps} reps ${deltaStr}`);
  }
  ...
  async function syncNow() {
    ...
    try {
      ...
      if (r.ok) {
        ...
        const localRev = meta?.rev || 0; const remoteRev = remoteMeta.rev || 0;
    ...
    } catch (e) { alert('Synk misslyckades: ' + e.message); } finally { setBusy(false); }
  }
  async function openHistory() {
    ...
    try { const r = await fetch(`/api/profiles/history?key=${blobKey}`); if (!r.ok) throw new Error('Kunde inte lista historik'); const data = await r.json(); setHistoryItems(Array.isArray(data.items) ? data.items : []); }
    catch (e) { alert(e.message); } finally { setHistoryBusy(false); }
  }
  async function restoreSnapshot(item) {
    try { const r = await fetch(item.url); if (!r.ok) throw new Error('Kunde inte hämta snapshot'); const data = await r.json(); if (Array.isArray(data.users)) setUsers(data.users); if (data.profileMeta) setMeta(data.profileMeta); setToast({ type:'up', msg:'Återställde snapshot lokalt. Spara till server för att skriva över.' }); }
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 text-slate-900">
          ...
          <div className="mx-auto max-w-6xl px-4 py-6 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm md:text-base text-white/95">6 veckor, 14 pass, kg‑beräkningar, FT‑stjärnor & quotes</p>
              </div>
            </div>
                  ...
                  return (
                    <article key={w.id} className={`rounded-2xl border shadow-sm bg-white p-4 ${log.done ? 'ring-2 ring-green-500/40' : ''}`}>
                      ...
                      {rows.some(r=>r.kind === "negative") && (
                        <div className="mt-3 text-xs text-slate-600">Negativt set: tung excentrisk 1x1 (~110% av arbets‑1RM). Träna säkert med spotter.</div>
                      )}
                      {rows.some(r=>r.kind === "max") && (
                        <div className="mt-3 text-xs text-slate-600">Max‑test: sikta runt ~115% av start‑1RM; FT‑utfall under vägen kan höja/sänka målet.</div>
                      )}
                    </article>
                  );
                })}
      ...
      <footer className="mx-auto max-w-6xl px-4 pb-10 text-xs text-slate-500">
        <div className="mt-6">Rev: {meta?.rev || 0} · Senast sparad: {meta?.lastSavedAt ? new Date(meta.lastSavedAt).toLocaleString() : '–'} · Device: {deviceId.slice(0,8)}… · App {APP_VERSION}</div>
      </footer>
    </div>
  );
}
...
function LiftEmoji() {
  return (
    <span className="inline-flex items-center justify-center text-3xl select-none" aria-hidden>🏋️‍♂️</span>
  );
}
